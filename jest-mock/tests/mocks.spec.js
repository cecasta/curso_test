/**
 * ref: https://github.com/joseluisgs/testing-js-jest.git
 */
/* eslint-disable no-undef */
import pizzas from '../src/pizzas.json';

describe('Jugando con Mocks desde cero para TDD', () => {
// Un test sencillo para saber si los datos de la pizza han cambiado
  ...
  // Podemos modificarla
  test('Espiando el uso de una función', () => {
    // Objeto con la función a mockear
    const pizza = {
      name: (n) => `Pizza name: ${n}`,
    };
    // Vamos a espiar esta función con este parámetro
  ...
  test('Spiar usando mockImplemntation', () => {
    ...
    spy.mockRestore(); // volvemos a la original
    expect(pizza.name('Cheese')).toBe('Pizza name: Cheese');
  });
